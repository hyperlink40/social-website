      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            {isConnected
              ? `Showing ${results.length} live results from BetiGolo API`
              : 'Match results from BetiGolo prediction data'}
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          {(['slideshow', 'table', 'raw_api'] as const).map(tab => (
            ...
            >
              ...
              {tab === 'raw_api' && <><FileJson className="w-3.5 h-3.5" /> API Response</>}
            </Button>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        {isConnected && (
          <div className="flex items-center gap-2 text-sm bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 px-3 py-1.5 rounded-full border border-emerald-200 dark:border-emerald-800">
            ...
            Connected — {results.length} results · {winRate}% win rate
          </div>
        )}
            ...
            <div className="relative">
              <Card className={`overflow-hidden border-2 transition-all duration-300 ${
                ...
                <div className={`px-6 py-3 text-sm font-semibold flex items-center gap-2 ${
                  ...
                  <Badge variant="premium" className="ml-2">BETIGOLO</Badge>
                  <span className="ml-auto text-slate-500 dark:text-slate-400 font-normal text-xs">
                    {current.date}
                  </span>
                </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="px-6 text-center">
                      ...
                      <div className="flex items-center justify-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-slate-400">Full Time</span>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 border-t border-slate-100 dark:border-slate-800 pt-4">
                    <div className="text-center">
                      <p className="text-xs text-slate-500 mb-1">AI Pick</p>
        ...
        <>
          <div className="flex items-center gap-2">
            ...
            {(['all', 'win', 'loss'] as const).map(f => (
              ...
              >
                {f === 'all' ? 'All Results' : f === 'win' ? '✅ Wins Only' : '❌ Losses Only'}
              </button>
            ))}
            {filter !== 'all' && (
                ...
                {filteredResults.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-4 py-8 text-center text-slate-500">
                      {loading ? 'Loading...' : results.length === 0 ? 'No results loaded. Click "Fetch from BetiGolo API".' : 'No matches match the current filter.'}
                    </td>
                  </tr>
                ) : (
                  filteredResults.map(r => (
                    <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <Badge variant="premium" className="ml-2 text-[10px]">BETIGOLO</Badge>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        {r.outcome === 'win'
                          ? <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-300 dark:border-green-700"><CheckCircle2 className="w-3 h-3" /> W</span>
                          : <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border border-red-300 dark:border-red-700"><XCircle className="w-3 h-3" /> L</span>
                        }
                      </td>
                    </tr>
                  ))
            ...
            <div className="px-4 py-3 border-t border-slate-100 dark:border-slate-800 text-xs text-slate-500 flex flex-wrap items-center justify-between gap-2">
              ...
              <span>
                ...
                <span className={`font-bold ${totalProfit / results.length >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {totalProfit / results.length >= 0 ? '+' : ''}
                  {(totalProfit / results.length).toFixed(2)}u
                </span>
                {' · '}Net P/L:{' '}
        ...
        <Card className="bg-gradient-to-r from-blue-600 to-indigo-700 border-none text-white">
          <CardContent className="p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-blue-100 text-sm">Get full access to BetiGolo API data, live predictions, and advanced analytics.</p>
            </div>
